...
my $fapiAttributeIdsFile = "./fapiAttributeIds.H";
my $fapiPlatAttributeServiceFile= "./fapiPlatAttributeService.H";
...
if (!open (FILE, $fapiAttributeIdsFile)) {
  if ($VERBOSE) { print "NOTE:: No $fapiAttributeIdsFile found. Exiting\n"; }
  exit;
}
...
my $systemRc = system("cp $servicePath/$fapiPlatAttributeServiceFile .");
if ($systemRc) {
...
open (FILE, $fapiPlatAttributeServiceFile) or die "ERROR:: could not open $fapiPlatAttributeServiceFile\n";
while (<FILE>) {
  if (m/\s*#define\s+(\w+)_GETMACRO\s+(\S+)\s*/) {
    $getMacros{$1} = $2;
    if ($DEBUG) { print "DEBUG:: attribute = $1 : GETMACRO = $2\n"; }
...
if (@newAttributeDefines != 0) {
  ...
  if ($insertTagFound == 0) {
    ...
    system("mv $updatedFapiPlatAttributeServiceFile $fapiPlatAttributeServiceFile");
  }
}
...
sub help {
  printf("Usage:  fapiAttributeUpdate.pl [-path <pathToFapiHeaders>] [-verbose|-v] [-help|-h]\n");
