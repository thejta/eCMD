...
uint32_t ecmdSimEXPECTFACUser(int argc, char * argv[]) {
  ...
  if (argc > 3) {
    ...
    int match = sscanf(argv[3], UINT64_DEC_FORMAT, &row);
    if (match != 1) {
      ecmdOutputError("Error occurred processing row!\n");
      return ECMD_INVALID_ARGS;
    }
  }
  if (argc > 4) {
...
uint32_t ecmdSimexpecttcfacUser(int argc, char * argv[]) {
  ...
  uint64_t row = 0;
  uint32_t startBit = 0, bitLength = 0;
  ...
  if (!useSubset) {
    if (argc > 2) {
      ...
      int match = sscanf(argv[2], UINT64_DEC_FORMAT, &row);
      if (match != 1) {
        ecmdOutputError("Error occurred processing row!\n");
        return ECMD_INVALID_ARGS;
      }
...
uint32_t ecmdSimgettcfacUser(int argc, char * argv[]) {
  ...
  if (!useSubset) {
    if (argc > 1) {
      ...
      int match = sscanf(argv[1], UINT64_DEC_FORMAT, &row);
      if (match != 1) {
        ecmdOutputError("Error occurred processing row!\n");
        return ECMD_INVALID_ARGS;
      }
